...
- **Les 4 fiches concrètes** (X2-X5, 2026-07-09), chacune via `FicheOverlay` + `.fiche-filrouge` en
  ...
  règles + dose du moment en quarts jour/nuit — **étendue en X8**, 2026-07-09 : dose de jour en vert +
  marge de quarts vides coloriables au stylo (patch en cours complété + 1 patch), impression forcée
  des aplats (`print-color-adjust: exact`) ; bloc optionnel « Ma prise ponctuelle » via un sélecteur
  dédié à la fiche (gomme/pastille/sublingual/spray) affichant `TechniqueIllustration` (image-ready,
  placeholder tant qu'aucune image fournie) + bonnes pratiques de la forme) ; **Mes raisons** (Motivation, bouton « Imprimer mes
