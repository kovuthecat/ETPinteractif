...
**Phase 2 — Corrections UX (audit du 28/06, `PLAN_corrections-ux.md`) : en cours.** C1 (socle visuel sémantique), C2 (titration substituts illimitée + patchs 2×2), C3 (nicotine : timeline animée + coloration par zone), C4 (soulagement : graphe stress + nicotine, récit proposé à valider) faits. Reste C5-C10.
...
1. **Lot `PLAN_corrections-ux.md` en cours.** C1, C2, C3, C4 faits. Suite recommandée par le plan (C4 traité hors ordre, à la demande de Thibault) : C5 → C6 → C9 → C8 → C7 → C10.
