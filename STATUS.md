...
**Phase 2 — Corrections UX (audit du 28/06, `PLAN_corrections-ux.md`) : en cours.** C1 (socle visuel sémantique), C2 (titration substituts illimitée + patchs 2×2), C3 (nicotine : timeline animée + coloration par zone) faits. Reste C4-C10.
...
- `src/styles/global.css` (C1) : cibles tactiles `min-height`/`min-width: var(--target-min)` sur `button`/contrôles ; classe utilitaire `.activeDoubled` (contour + ✓ + couleur via `--active-color`, jamais la couleur seule, pas encore consommée) ; bloc `prefers-reduced-motion` neutralisant animations/transitions. Tokens sémantiques consommés depuis C2 (Module 3 : `--color-confort`/`--color-toxique`/`--color-nav`). Reste C3-C9.

### Logique pure

- `src/lib/nicotineCurve.ts` (T5, **C3**) : `sampleCurve` (composition cigarette/ponctuel/vapoteuse/patch — patch désormais aussi disponible comme event ancré `patchKernel(x, t0)`, valeurs relatives 0–1 clampées), `classifyZone(y)` (manque/confort/haut, frontières incluses dans confort) et `toSvgPath`. Couvert par tests Vitest (`nicotineCurve.test.ts`, 10 tests verts). Partagé par Module 2 (T6/C3) et Module 5 (T7, inchangé — utilise toujours `patch: false` + events cigarette).
...
1. **Lot `PLAN_corrections-ux.md` en cours.** C1, C2, C3 faits. Suite recommandée par le plan : C5 → C6 → C9 → C8 → C4 → C7 → C10.
