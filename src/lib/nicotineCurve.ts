...
export type CurveEvent = { kind: 'cigarette' | 'ponctuel' | 'vapoteuse'; t: number };
...
function patchValue(x: number): number {
  if (x < 0.1) return PATCH_PLATEAU * (x / 0.1);
